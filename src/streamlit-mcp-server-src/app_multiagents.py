...
from multi_agents_manager import MultiAgentsManager
...
if "session_initialized" not in st.session_state:
    ...
    st.session_state.multi_agents_manager = None  # MultiAgentsManagerインスタンス
    st.session_state.history = []
    st.session_state.timeout_seconds = 120
    st.session_state.selected_model = "gpt-4o-mini" # "claude-3-7-sonnet-latest"
...
async def process_query(query, text_placeholder, tool_placeholder, timeout_seconds=60):
    """
    MultiAgentsManager経由でsupervisor-workerパターンでクエリ処理
    """
    try:
        manager = st.session_state.get("multi_agents_manager", None)
        if manager is None:
            return ( {"error": "🚫 Agent manager has not been initialized."}, "🚫 Agent manager has not been initialized.", "" )
        
        # ストリーミング表示用のダミー処理（実際のストリーミングは後で実装）
        text_placeholder.markdown("🤖 エージェントが処理中です...")
        
        try:
            # タイムアウト付きでクエリ処理（thread_idを渡す）
            response = await asyncio.wait_for(
                manager.process_query(query, thread_id=st.session_state.thread_id),
...
async def initialize_session(mcp_config=None):
    """
    MultiAgentsManagerを使って初期化
    """
    with st.spinner("🔄 Connecting to MCP server..."):
        if mcp_config is None:
            mcp_config = load_config_from_json()
        manager = MultiAgentsManager(MultiServerMCPClient, OUTPUT_TOKEN_INFO, default_model=st.session_state.selected_model)
        await manager.initialize(mcp_config, selected_model=st.session_state.selected_model)
        st.session_state.multi_agents_manager = manager
...
with st.sidebar:
    ...
    # --- エージェント選択UIは削除（supervisorが自動選択するため） ---
    st.info("🤖 エージェントは自動選択されます（supervisor-workerパターン）")
...
with st.sidebar:
    ...
    if st.session_state.multi_agents_manager:
        st.write(f"🛠️ MCP Tools Count: {st.session_state.multi_agents_manager.get_tool_count()}")
    ...
    if st.button(
        ...
        with apply_status.container():
            ...
            st.session_state.session_initialized = False
            st.session_state.multi_agents_manager = None
