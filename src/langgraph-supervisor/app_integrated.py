...
def load_config_from_json():
    ...
    default_config = {
        ...
        "hotpepper_search": {
            "command": "python",
            "args": ["./mcp_server_hotpepper.py"],
            "transport": "stdio"
        },
        "google_maps": {
            "command": "python",
            "args": ["./mcp_server_googlemaps.py"],
            "transport": "stdio"
        },
        "search": {
            "command": "python",
    ...
    with st.spinner("🔄 Connecting to MCP server..."):
        ...
        for server_name, server_config in mcp_config.items():
            if "command" in server_config and server_config["command"] == "python":
                script_path = server_config["args"][0] if server_config["args"] else ""
                if not os.path.exists(script_path):
                    missing_files.append(script_path)
        
        if missing_files:
            st.error(f"❌ MCP server files not found: {', '.join(missing_files)}")
            return False
                            ...
                            st.markdown(
                                ...
                                ">
                                    <iframe 
                                        ...
                                        height="200" 
                                        frameborder="0" 
                                        style="border-radius: 12px; background: #191414;" 
