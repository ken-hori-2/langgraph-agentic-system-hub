...
def run_agent(messages):
    ...
    try:
        ...
        import streamlit as st
        st.write("【DEBUG: app.invoke result】", result)
        text = result.get("message")
        if not text and "error" in result:
            text = result["error"]
        # 構造化Spotifyリストを抽出
        results = result.get("results")
        # messages内のdictやcontentがJSON文字列の場合もパースしてresultsを抽出
        if (not results) and "messages" in result and result["messages"]:
            for msg in reversed(result["messages"]):
                role = msg.get("role") if isinstance(msg, dict) else getattr(msg, "role", None)
                content = msg.get("content") if isinstance(msg, dict) else getattr(msg, "content", None)
                st.write(f"【DEBUG: role={role}, content型={type(content)}, content={content}】")
                # dict型でresultsキーがある場合
                if isinstance(msg, dict) and "results" in msg:
                    results = msg["results"]
                # contentがJSON文字列でresultsキーがある場合
                if isinstance(content, str):
                    try:
                        ...
                        pass
                # text抽出
                if not text and role in ("assistant", "ai") and content:
                    text = extract_text(content)
        st.write("【DEBUG: 抽出したtext】", text)
        st.write("【DEBUG: 抽出したresults】", results)
        return {
